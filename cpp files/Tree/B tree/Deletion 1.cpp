...
void mergeNodeWithParent(bdptr &BD, bdptr &parent, int parentIndex, int index, bdptr &Head)
{
...
void deleteNode(bdptr &BD, bdptr &parent, int parentIndex, int data, bdptr &Head)
{
    if(BD==NULL)
        return;
    ...
    for(i=0;i<BD->cnt;i++)
    {
        ...
        else if(data == BD->keys[i])
        {
            ...
            if(isLeaf(BD))
            {
                ...
                {
                    mergeNodeWithParent(BD, parent, parentIndex, i, Head);
                }
            }
            else
...
void Delete(bdptr &BD, int data)
{
    ...
    bdptr parent = NULL;
    int parentIndex = 0;
    deleteNode(BD, parent, parentIndex, data, BD);
}
