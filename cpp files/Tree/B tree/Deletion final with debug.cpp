...
bool keyDistribution(bdptr &BD, bdptr &parent, int parentIndex, int data, int index)
{
    ...
    else if( (parent!=NULL) && (parentIndex<M-1) && (parent->cptr[parentIndex+1] != NULL) && (parent->cptr[parentIndex+1]->cnt > MIN_KEY) )
    {
        ...
        cout<<"next sibling childs before"<<endl;
        for(int i=0;i<=nextSibling->cnt;i++)
           {  cout<<"child "<<i<<" = "; printNode(nextSibling->cptr[i]); }
        ...
        nextSibling->cnt--;
        cout<<"next sibling childs after"<<endl;
        for(int i=0;i<=nextSibling->cnt;i++)
           { cout<<"child "<<i<<" = "; printNode(nextSibling->cptr[i]); }
        return true;
    }
...
{
    int currDepth = DEPTH++;
    cout<<"Depth st = "<< (currDepth) <<" parentIndex = "<<parentIndex<<" data = "<<data<<" merged = "<<merged<<" mergedIndex = "<<mergedParentIndex<<endl;
    if(BD==NULL)
        return;
        ...
        else if(data == BD->keys[i])
        {
            ...
            if(isLeaf(BD))
            {
                ...
                else if( !keyDistribution(BD, parent, parentIndex, data, i) )
                {
                    ...
                    cout<<"Before calling merge node"<<endl;
                    mergeNodeWithParent(BD, parent, parentIndex, i, Head, mergedParentIndex);
                    cout<<"after calling merge node"<<endl;
                    merged = true;
                }
                else
                    cout<<"Key distribution has been done "<<endl;
    ...
    cout<<"Depth end = "<< currDepth <<" parentIndex = "<<parentIndex<<" data = "<<data<<" merged = "<<merged<<" mergedIndex = "<<mergedParentIndex<<endl;
    return;
}
